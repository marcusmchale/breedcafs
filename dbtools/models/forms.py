...
from dbtools.models.user import get_unregistered_emails
...
class RemoveUserEmailForm(FlaskForm):
	...
	def update(username):
		...
		form.emails_list.choices = [
			(i, i) for i in get_unregistered_emails(User(username).username)
		]
		return form
...
class UserAdminForm(FlaskForm):
	...
	def update(
		...
		users = [record[0] for record in User(user).get_users_affiliations(access)]
		for user in users:
