...
from .record import Record, Property, Trait, Condition, Curve
import copy
#import grp
from app.cypher import Cypher
from app.emails import send_email
from app.models.parsers import Parsers
from app.models.property_manager import PropertyUpdater
...
class CSVTable(RecordSubmission):
	...
	def get_input_variables(self):
		...
		input_variables = Query().get_list(query, parameters)
		if not input_variables:
			...
			input_variables = [Query().get_bolt(query, parameters).single()[0]]
		if not input_variables:
			logging.info(
		...
		with open(self.file_path) as csv_file:
			...
			while True:
				...
				try:
					...
					if self.row_errors:
						self.revert_submitted()
				except StopIteration:
					break

			self.provide_report()
			self.confirm_merger()  # function to remove the record_for property "merging"
	...
	def submit_batch(self, inputs_records, batch_id):
		...
		for i in self.input_variables:
			...
			# todo consider getting a lock on the item node in this check
			#  in case another transaction updates the source before this transaction is committed
			elif input_name in [
				'assign variety name'
			...
			if self.record_type == 'property':
				self.update_properties(tx, batch_id, input_name)
	...
	def update_properties(self, tx, batch_id, input_name):
	...
	def confirm_uid_set(self, uid_set):
		...
		missing_uids = [str(i).upper() for i in Query().get_list(query, parameters)]
		if missing_uids:
			self.file_errors.add('The following UIDs were not found in the database:\n %s' % '\n'.join(missing_uids))


#	def confirm_source_block(self, tx, input_name, records):
#		# For tree and block to block assignment
...
#			'	CALL apoc.path.expand(s, "IS_IN> | FROM>", "/Item", 1, 2) '
#			'		yield path '
#			'	WITH '
#			'		record_data, '
#			'		field, '
...
#			#   2: only a single variety specification in each new provenance path
#			'	OPTIONAL MATCH '
#			'		ancestry = (field)<-[:IS_IN*]-(new_source: Block {%s: record_data.value}) '
#			'	WITH '
#			'		record_data, '
#			'		item, '
#			'		CASE WHEN new_source IS NULL THEN False ELSE True END as new_source_found, '
#			'		EXISTS ((current_source)<-[:IS_IN*]-(new_source)) as new_source_valid, '  # confirms 1
#			'		[ '
#			'			i IN nodes(ancestry) '
#			'			WHERE i.uid IS NOT NULL '
#			# NB: the below filtering on variety is unnecessary but reduces the size of ancestors to handle,
#			# if considering other properties to enforce for lineage then remove this line and similar below it
#			'			AND i.variety IS NOT NULL '
#			'		] as ancestors '
#			'	CALL apoc.path.expandConfig(item, { '
#			'		relationshipFilter: "<FROM | <IS_IN", '
#			'		labelFilter: ">Item", '
#			'		optional: True '
#			'	}) yield path '
#			# NB: multiple descendants paths are typical, hence this below indent to reflect the need to later collect these per row_index
#			# each provenance path must be assessed for variety conflicts independently and later collected
#			'		WITH '
#			'			record_data, '
#			'			new_source_found, '
#			'			new_source_valid, '
#			'			ancestors + coalesce('
#			'				[i in nodes(path) WHERE i.uid IS NOT NULL AND i.variety IS NOT NULL], '
#			'				CASE WHEN item.variety IS NULL THEN [] ELSE [item] END, '
#			'			) as lineage '
#			'		UNWIND '
#			'			lineage as kin '
#			'		WITH '
...
#			'			new_source_found, '
#			'			new_source_valid, '
#			'			[i in collect([uid_variety, variety_conflicts_found]) WHERE i[1] | i[0]] as variety_conflicts '
#			'	RETURN  { '
#			'		row_index: record_data.row_index, '
#			'		found: new_source_found, '
#			'		valid: new_source_valid, '
#			# list of lineage specific lists of uid and variety specification where conflict is found
...
#		}
	...
	def confirm_source_assignment(self, tx, input_name, records):
		...
		query = (
				...
				# now expand to find descendants
				'		CALL apoc.path.expandConfig(item, { '
				'			relationshipFilter: "<FROM | <IS_IN", '
				'			labelFilter: ">Item", '
				'			optional: True '
				'		}) yield path '
				# NB: multiple descendants paths are typical (these are also factorial with the ancestor paths in creating lineage paths)
				# each lineage path must be assessed for variety conflicts independently and later collected
				'			WITH ' 
				'				record_data, '
				'				identifier_value, '
				'				new_source_found, '
				'				new_source_valid, '
				'				variety_ancestors + coalesce('
				'					[i in nodes(path) WHERE i.variety IS NOT NULL], '
				'					CASE WHEN item.variety IS NULL THEN [] ELSE [item] END, '
				'				) as lineage '
				'			UNWIND '
				'				lineage as kin '
	...
	def confirm_variety_assignment(self, tx, input_name, records):
		...
		query = (
				...
				'	MATCH '
				'		ancestor_path = (item)-[:FROM|IS_IN*]->(field) '
				'	CALL apoc.path.expandConfig(item, {'
				'			relationshipFilter: "<FROM | <IS_IN", '
				'			labelFilter: ">Item", '
				'			optional: True '
				...
				'		record_data, '	
				'		coalesce( '
				'			[i IN nodes(ancestor_path) WHERE i.variety IS NOT NULL], '
				'			CASE WHEN field.variety IS NOT NULL THEN [field] ELSE [] END'
				'		) as ancestors,'
				'		coalesce( '
				'			[i IN nodes(path) WHERE i.variety IS NOT NULL], '
				'			[] '	
				...
				'	WITH '
				'		record_data, '
				'		ancestors + descendants as lineage '
	...
	def merge_records(self, tx, input_name, batch_id, records):
		...
		query += (
			...
			'	record.value = record_data.value '
			'	record_for.merging = True ' # use this property to filter out currently merging data in download etc.
			' ON MATCH SET '
			'	record.found = True '
	...
	def check_record_for_conflicts(self, input_name, record):
		...
		if 'access' in record and not record['access']:
			self.add_row_error(
				...
				'Please contact them to arrange access if you wish to continue merging these records. '
				% record['submitted_by']
			)
			return
...
# TODO CONSIDER PROPERTY REVERSION!
