...
class LocationForm(FlaskForm):
	...
	def update(optional = False):
		...
		form.region.choices = [('','Select Region')] + [(REGIONS[i].lower(), REGIONS[i]) for i, items in enumerate(REGIONS)]
		form.farm.choices = [('','Select Farm')] + [(FARMS[i].lower(), FARMS[i]) for i, items in enumerate(FARMS)]
...
class AddPlot(FlaskForm):
	...
	text_plot = StringField('Plot text input',
		[InputRequired(),
		Regexp('([^\x00-\x7F]|\w|\s)+$', message='Plot name contains illegal characters'),
		Length(min=1, max=50, message='Maximum 50 characters')],
		filters=[strip_filter],
