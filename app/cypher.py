...
#sample procedures
	...
	upload_FB_block = ( 
		...
		' LOAD CSV WITH HEADERS FROM $filename as csvLine '
		# And identify the plots and traits assessed
		' MATCH (plot:Plot {uid:toInteger(head(split(csvLine.UID, "_")))})<-[:IS_IN]-(pb:PlotBlocks), '
		...
		' MERGE (fb)-[:SUBMITTED]->(upt:UserPlotTrait)<-[:CONTRIBUTED_BY]-(pt) '
		' MERGE (upt)-[:SUBMITTED]->(ubt:UserBlockTrait)<-[:CONTRIBUTED_BY]-(bt) '
		' MERGE (ubt)-[s1:SUBMITTED]->(d) '
			' ON CREATE SET s1.time = timestamp() '
		#And give the user feedback on their submission success
		' RETURN d.found' 
		)
	upload_FB_plot = ( 
		...
		' MATCH (:Plot {uid:toInteger(csvLine.UID)}),'
			...
			' (:User {username : $username}) '
				...
				' -[:SUBMITTED]->(fb:FieldBook)'
		# Crete per plot per trait node
		' MERGE (plot)<-[:FROM_PLOT]-(pt:PlotPlotTrait)-[:FOR_TRAIT]-(trait) '
		# Merge the data point linking to the blocktrait node
		' MERGE (d:Data {block:csvLine.UID, '
					' value:csvLine.value, '
					' timeFB:csvLine.timestamp, '
			...
			' ON CREATE SET d.found = false '
			' ON MATCH SET d.found = true '
		# track user submissions through UserPlotTrait container
		' MERGE (fb)-[:SUBMITTED]->(upt:UserPlotTrait)<-[:CONTRIBUTED_BY]-(pt) '
		' MERGE (upt)-[s1:SUBMITTED]->(d) '
			' ON CREATE SET s1.time = timestamp() '
		#And give the user feedback on their submission success
		' RETURN d.found' 
		)
	...
	get_submissions_range = ( 
		...
		#Samples
		' MATCH (:User {username:$username})-[:SUBMITTED*..10] '
			' ->(sample:Sample)-[:FROM_TREE]->(:TreeSamples) '
			' -[:FROM_TREE]->(:Tree) '
			' -[r:IS_IN]->(:BlockTrees) '
			' -[:IS_IN]->(block:Block) '
		' RETURN '
			...
			' (id(block) + "sample_count_node") as d_id, '
			' "Block" as n_label, '
			' block.name as n_name, '
			' id(block) as n_id, '
			' "FROM_BLOCK" as r_type,  '
			' id(r) as r_id, '
			' (id(block) + "sample_count_node") as r_start, '
			' (id(block) + "tree_count_node") as r_end'
		' UNION '
		#sample data
		#tree data (from block)
		' MATCH (:User {username:$username})-[:SUBMITTED*..10] '
		' ->(data:Data)-[:DATA_FOR]->(ss:SampleSampleTrait) '
		' -[:FROM_TREE]->(:Tree) '
		' -[:IS_IN]->(:BlockTrees) '
		' -[q:IS_IN]->(block:Block), '
		' (tt)-[:FOR_TRAIT]->(pt:PlotTreeTrait) '
		...
		' RETURN '
			...
			' id(pt) as r_start, '
			#' id(plot) as r_end '
			' (id(plot) + "tree_count_node") as r_end '
		' UNION '
		#tree data (from block)
		' MATCH (:User {username:$username})-[:SUBMITTED*..10] '
		' ->(data:Data)-[:DATA_FOR]->(tt:TreeTreeTrait) '
