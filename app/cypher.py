class Cypher:
	...
	upload_check_value = (
		...
		'					AND toInteger(trim(split(value, ";")[1])) <=24 '
		'						THEN trim(value) '
		'					ELSE Null '
		'					END '
		'			ELSE '
	...
	upload_table = (
		...
		' WITH '
		'	trim(toUpper(csvLine.UID)) as uid, '
		'	trim(csvLine.date) as date, '
		'	trim(csvLine.time) as time '
		...
		'		item, trait, ' 
		'		plot_item_plot, tree_item_tree, tree_item_plot, '
		'		uid, person, date, time, '
		'		trim(csvLine[trait_name]) as value '
		'	FOREACH (n in CASE '
		'		WHEN trait.level = "plot" '
		'			THEN [1] ELSE [] END | '
		'				MERGE '
