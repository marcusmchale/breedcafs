class Cypher:
	...
	upload_FB = (
		...
		' OPTIONAL MATCH '
		'	(item)-[:FROM_PLOT*2]->(plot_item_plot:Plot) '
		' OPTIONAL MATCH '
		'	(item)-[:FROM_TREE*2]->(tree_item_tree:Tree) '
		'	-[:IS_IN*2]->(tree_item_plot:Plot) '
		' FOREACH (n in CASE '
		'	WHEN trait.level in ["farm","plot"] '
		'		THEN '
		'			MERGE '
		...
		'				(item)<-[:FOR_ITEM]-()-[: FOR_TRAIT]->(pt) '
		' ) '
		# get rid of the optional matches since they are no longer needed
		' WITH item, trait, person, location, time, value '
		...
		# and the item/trait container
		'	(item)<-[: FOR_ITEM]-(it)-[: FOR_TRAIT*]->(: Trait)  '
		...
		'			MERGE '
		# If d.value is not False then variety should already exist so safe to merge here
		'				(variety:Variety { '
		'					name_lower: toLower(d.value)'
		'				}) '
		'			MERGE '
		...
		'	AND trait.name_lower = "branch id" '
		'	THEN [1] ELSE [] END | '
		'		MERGE '
		# This will create an orphan branch if branch is not found
		# so only link if not created and flag branch for deletion
		# but shouldn't conflict with creating 
		# new branches since these are specified by UID
		'			(branch:Branch { '
		'				id: d.value'
		'			}) '
		'			-[:FROM_TREE*2]-() '
		'			-[:FROM_PLOT*2]-(tree_item_plot) '
		'			ON CREATE SET '
		'				branch.found = False '
		'				branch.delete_me = True '
		'				temp.delete_me = True '
		'			ON MATCH SET '
		'				branch.found = True '
		'		FOREACH (n in CASE '
		'			WHEN branch.found = True '
		'				THEN [1] ELSE [] END | '
		'					MERGE (item)-[:FROM_BRANCH]->(branch) '
		'		) '
		' )'
		# if trait is leaf id then link to leaf
		' FOREACH (n IN CASE '
		'	WHEN trait.level = "sample" '
		'	AND trait.name_lower = "leaf id" '
		'	THEN [1] ELSE [] END | '
		'		MERGE '
		# This will create an orphan leaf if leaf ID is not found
		# so only link if not created and flag leaf for deletion
		# but shouldn't conflict with creating 
		# new branches since these are specified by UID
		'			(leaf:Leaf { '
		'				id: d.value'
	...
	upload_FB_leaf = (
		...
		' MERGE '
		'	(plot)<-[: FROM_PLOT]-(plt: PlotLeafTrait)'
		'	-[: FOR_TRAIT]->(trait) '
		# Then  /Tree/LeafTrait container node
		' MERGE '
		'	(tree)<-[: FROM_TREE]-(tlt: TreeLeafTrait)'
		'	-[: DATA_FOR]->(plt) '
		# And finally /Leaf/LeafTrait container node
		' MERGE '
		'	(leaf)<-[: FROM_LEAF]-(llt: LeafLeafTrait)'
		'	-[: DATA_FOR]->(tlt) '
		# Merge the data point linking to Sample/SampleTrait node
		' MERGE '
		'	(d:Data { '
		...
		# track user submissions through /User/Plot/LeafTrait container
		' MERGE '
		'	(fb)-[: SUBMITTED]->(uplt: UserPlotLeafTrait)'
		'	-[: CONTRIBUTED]->(plt) '
		' MERGE '
		'	(uplt)-[s1: SUBMITTED]->(d) '
		'	ON CREATE SET s1.time = timestamp() '
	...
	upload_FB_sample = (
		...
		' MERGE '
		'	(plot)<-[: FROM_PLOT]-(pst: PlotSampleTrait) '
		'	-[: FOR_TRAIT]->(trait) '
		# Then  /Tree/SampleTrait container node
		' MERGE '
		'	(tree)<-[: FROM_TREE]-(tst: TreeSampleTrait) '
		'	-[: DATA_FOR]->(pst)'
		# And finally /Sample/SampleTrait container node
		' MERGE '
		'	(sample)<-[: FROM_SAMPLE]-(sst: SampleSampleTrait) '
		'	-[: DATA_FOR]->(tst)'
		# Merge the data point linking to Sample/SampleTrait node
		' MERGE '
		'	(d:Data { '
