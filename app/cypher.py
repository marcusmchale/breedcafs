class Cypher:
	...
	upload_check_value = (
		...
		'					ELSE Null '
		'					END '
		'			WHEN feature.name_lower = "variety code" '
		'				THEN CASE '
		'					WHEN toLower(value) in extract(item in feature.category_list | toLower(item)) '
		'						THEN trim(value) '
		'					ELSE Null '
	...
	shared_upload_code = (
		...
		' OPTIONAL MATCH '
		'	(item)-[from_current: FROM]->(:ItemSamples)-[:FROM]->(:Item) '
		...
		'	THEN [1] ELSE [] END | '
		'		FOREACH (from_rel in from_current | '
		'			DELETE from_rel '
		'		) '
	...
	#upload_table_feature_format_check = (
	#	' LOAD CSV WITH HEADERS FROM $filename as csvLine '
	#	' WITH '
	#	'	csvLine, '
	#	'	csvLine.row_index as row_index, '
	#	'	split(trim(toUpper(csvLine.uid)), ".")[0] as uid '
	#	' OPTIONAL MATCH  '
	#	'	(item: Item { '
	#	'		uid: '
	...
	#	'		row_index, '
	#	'		CASE WHEN item IS NULL THEN False ELSE True END as item, '
	#	'		CASE WHEN feature IS NULL THEN False ELSE feature.name_lower END as feature, '
	#	'		feature.format as format, '
	#	'		feature.category_list as category_list,'
	#	'		CASE WHEN item IS NOT NULL AND feature IS NOT NULL '
	#	'			THEN '
	#	'				CASE WHEN ' + upload_check_value +
	#	'				IS NULL THEN False ELSE True END '
	#	'			ELSE False '
	...
	upload_table_condition_conflict_check = (
		' LOAD CSV WITH HEADERS FROM $filename as csvLine '
		' WITH '
		'	csvLine, '
		...
		'	WITH '
		'		row_index, '
		'		item, feature, feature_name, start, end, '
		...
		'	OPTIONAL MATCH '
		'		(p)<-[a: AFFILIATED {confirmed: true}]-(: User {username_lower: toLower($username)}) '
		...
		# there may be implications with multiple records for the same item in a single file
		# '	SET if._LOCK_ = True '
		'	WITH '
		'		row_index, '
		'		feature_name, '
		...
		'		CASE WHEN a.access THEN r.value ELSE "ACCESS DENIED" END AS existing_value, '
		'		s.time AS submitted_at, '
		'		CASE WHEN a.access THEN u.name ELSE p.name END AS user, '
		'		a.access AS access '
		'	WHERE '
		'	( '
		'		item IS NULL '
