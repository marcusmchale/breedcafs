...
#sample procedures
	...
	upload_FB_plot = ( 
		...
		' LOAD CSV WITH HEADERS FROM $filename as csvLine '
		# And identify the plots and traits assessed
		' MATCH (:Plot {uid:toInteger(csvLine.UID)}),'
	...
	get_submissions_range = ( 
		...
		# >=$starttime  <=$endtime
		#regions
		' MATCH (:User {username:$username})-[:SUBMITTED*4] '
		...
		' RETURN '
			...
			' id(plot) as r_end'
		' UNION '
		#trees (in plot but not block)
		' MATCH (:User {username:$username})-[:SUBMITTED*5] '
			' ->(tree:Tree)-[:IS_IN*2]->(plot:Plot) '
		' WHERE NOT (tree)-[:IS_IN]-(:BlockTrees) '
		' RETURN '
			' "Trees" as d_label, '
			' count(tree) as d_name, '
			' (id(plot)+ "_tree_count_node") as d_id, '
			' "Plot" as n_label, '
			' plot.name as n_name, '
			' id(plot) as n_id, '
			' "IN_PLOT" as r_type,  '
			' (id(plot)+ "_tree_count_rel") as r_id, '
			' (id(plot)+ "_tree_count_node") as r_start, '
			' id(plot) as r_end'
		' UNION '
		#trees (in block)
		' MATCH (:User {username:$username})-[:SUBMITTED*5] '
			' ->(tree:Tree)-[:IS_IN*2]->(block:Block) '
		' RETURN '
			' "Trees" as d_label, '
			' count(tree) as d_name, '
			' (id(block) + "_tree_count_node") as d_id, '
			' "Block" as n_label, '
			' block.name as n_name, '
			' id(block) as n_id, '
			' "IN_BLOCK" as r_type,  '
			' (id(block) + "_tree_count_rel") as r_id, '
			' (id(block) + "_tree_count_node") as r_start, '
			' id(block) as r_end'
		' UNION '
		#Samples (in plot but not block)
		' MATCH (:User {username:$username})-[:SUBMITTED*6] '
			' ->(sample:Sample)-[:FROM_TREE*2]->(tree:Tree) '
			' -[:IS_IN*2]->(plot:Plot) '
		' WHERE NOT (tree)-[:IS_IN]->(:BlockTrees) '
		' RETURN '
			' "Samples" as d_label, '
			' count(sample) as d_name, '
			' (id(plot) + "_sample_count_node") as d_id, '
			' "Plot" as n_label, '
			' plot.name as n_name, '
			' id(plot) as n_id, '
			' "FROM_PLOT_TREES" as r_type,  '
			' (id(plot) + "_sample_count_rel") as r_id, '
			' (id(plot) + "_sample_count_node") as r_start, '
			' (id(plot) + "_tree_count_node") as r_end'
		' UNION '
		#Samples (in block)
		' MATCH (:User {username:$username})-[:SUBMITTED*6] '
			' ->(sample:Sample)-[:FROM_TREE*2]->(:Tree) '
			' -[:IS_IN*2]->(block:Block) '
		' RETURN '
			' "Samples" as d_label, '
			' count(sample) as d_name, '
			' (id(block) + "_sample_count_node") as d_id, '
			' "Block" as n_label, '
			' block.name as n_name, '
			' id(block) as n_id, '
			' "FROM_BLOCK_TREES" as r_type,  '
			' (id(block) + "_sample_count_rel") as r_id, '
			' (id(block) + "_sample_count_node") as r_start, '
			' (id(block) + "_tree_count_node") as r_end'
		' UNION '
		...
		' RETURN '
			' "Trait" as d_label, '
			' (sampletrait.name + " (" + count(data) +")") as d_name, '
			' (id(plot) + "_" + id(sampletrait)) as d_id, '
			' "" as n_label, '
			' "" as n_name, ' #empty name means this node is removed before D3 so won't overwrite existing node
			' "" as n_id,'
			' "FROM_PLOT_SAMPLES" as r_type,  '
			' (id(plot) + "_" + id(sampletrait) + "_data_count_node") as r_id, '
			' (id(plot) + "_" + id(sampletrait)) as r_start, '
			' (id(plot) + "_sample_count_node") as r_end '
		' UNION '
		...
		' MATCH (:User {username:$username}) '
			...
			' RETURN '
				...
				' "" as n_id, '
				' "FROM_PLOT_TREES" as r_type,  '
				' (id(plot) + "_" + id(treetrait) + "_data_count_node") as r_id, '
				' id(ptt) as r_start, '
				' (id(plot) + "_tree_count_node") as r_end ' )
		#' UNION '
		##tree data (from block)
		#' MATCH (:User {username:$username})-[:SUBMITTED*..10] '
		#' ->(data:Data)-[:DATA_FOR]->(tt:TreeTreeTrait) '
		#' -[:FROM_TREE]->(:Tree) '
		#' -[:IS_IN]->(:BlockTrees) '
		#' -[q:IS_IN]->(block:Block), '
		#' (tt)-[:DATA_FOR]->(pt:PlotTreeTrait) '
		...
		#	' id(r) + id(q) as r_id, '
		#	' id(pt) + id(q) as r_start, '
		#	' (id(block) + "tree_count_node") as r_end '
		#' UNION '
		##block data
		#' MATCH (:User {username:$username})-[:SUBMITTED*..10] '
		#' ->(data:Data)-[:DATA_FOR]->(:BlockBlockTrait) '
		#' -[:DATA_FOR]->(pt:PlotBlockTrait) '
