class Cypher:
	...
	samples_add = (
		...
		' WITH '
		'	tree, tissue, storage, samples, id, field_samples, field, f, r, c '
		' ORDER BY '
		'	tree.id '
		...
		' MERGE '
		'	(user_field_samples)-[:SUBMITTED]->(user_tree_samples:UserTreeSamples)-[:CONTRIBUTED]->(tree_samples) '
		' WITH '
		'	id, '
		'	tree_samples, '
		'	field_tissue_storage, '
		'	user_tree_samples, '
		'	tree, '
		...
		# and the FieldTissueStorage container
		'	MERGE '
		'		(s)-[: COLLECTED_AS]->(field_tissue_storage) '
	...
	shared_upload_code = (
		...
		# '			MERGE '
		# '				(tree_item_tree)<-[:FROM_TREE]-(tree_trait)-[: FOR_TRAIT]->(field_trait) '
		'			MERGE '
		'				(item)<-[:FOR_ITEM]-(:ItemTrait)-[: FOR_TRAIT]->(field_trait) '
		' ) '
	...
	upload_fb = (
		...
		' OPTIONAL MATCH '
		'	(item)-[:IS_IN*2]->(field_item_field:Field) '
		' OPTIONAL MATCH '
		'	(item)-[:FROM_TREE*2]->(tree_item_tree:Tree) '
		'	-[:IS_IN*2]->(tree_item_field:Field) '
		' WITH '
		'	item, trait, '
		'	field_item_field, tree_item_trail, tree_item_tree, '
