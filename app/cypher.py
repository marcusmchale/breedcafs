...
#sample procedures
	...
	get_submissions_range = ( ' MATCH (:User {username:$username}) '
		...
		' OPTIONAL MATCH (d)-[:IS_IN*..4]->()-[rel_location:IS_IN]->(location) '
		' OPTIONAL MATCH (d)-[:DATA_FOR]->()-[:FOR_TRAIT*2]->()-[rel_trait:FOR_TRAIT]->(trait) '
		' OPTIONAL MATCH (d)-[FROM_PLOT*]->()-[rel_plot:FROM_PLOT]->(plot) '
		...
		' RETURN '
			...
			' coalesce( '
					...
					' (labels(plot)[0]) '
				' ) as n_label, '
			' coalesce(location.name,trait.name,plot.name) as n_name,'
			' coalesce( '
					' id(location), '
					' id(trait), '
					' id(plot) '
			...
			' coalesce( '
				...
				' ) as r_type, '
			' coalesce( '
					' id(startNode(rel_location)), '
					' id(startNode(rel_trait)), '
					' id(startNode(rel_plot)) '
				' ) as r_start, '
			' coalesce( '
					' id(endNode(rel_location)), '
					' id(endNode(rel_trait)), '
					' id(endNode(rel_plot)) '
				' ) as r_end '
		'LIMIT 100' )
