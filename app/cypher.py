class Cypher:
	...
	upload_fb_check = (
			...
			'	toLower(trim(csvLine.trait)) as feature_name, '
			...
			'	`Category list`: feature.category_list, '
			'	Value: value, '
			'	Conflicts: conflicts '
			' } '
			' ORDER BY row_index '
	...
	upload_table_curve_check = (
		...
		'	csvLine, '
		'	['
		'		key in keys(csvLine) WHERE toFloat(key) in $x_values AND toFloat(csvLine[key]) IS NOT NULL '
		...
		'	-[:AFFILIATED {data_shared: true}]->(p:Partner) '
		' WHERE '
		'	size([x in $x_values WHERE x IN r.x_values]) > 0 '
		' OPTIONAL MATCH '
		'	(p)<-[a: AFFILIATED]-(: User {username_lower: toLower($username)}) '
		' WITH '
		'	row_index, '
	...
	upload_table_curve = (
		...
		' WITH '
		'	csvLine, '
		'	[key in keys(csvLine) WHERE toFloat(key) in $x_values | [toFloat(key), toFloat(csvLine[key])]] as x_y_list '
		' UNWIND x_y_list as x_y '
		' WITH '
		'	csvLine, '
		'	x_y '
