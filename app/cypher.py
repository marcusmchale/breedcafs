class Cypher:
	...
	get_trials_treecount = (
		...
		' OPTIONAL MATCH (trial)<-[:IS_IN]-(:TrialTrees)<-[:FOR]-(T:Counter {name:"tree"}) '
		' OPTIONAL MATCH (trial)<-[:IS_IN*2]-(block:Block) '
		' OPTIONAL MATCH (block)<-[:IS_IN*2]-(tree:Tree) '
		' WITH '
		'	country, '
		'	region, '
		'	farm, '
		...
		'		name: trial.name, '
		'		label:"Trial", '
		'		treecount: tree.count - sum(blocktrees), '
