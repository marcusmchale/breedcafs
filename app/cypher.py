class Cypher:
	...
	shared_upload_code = (
		...
		'	AND source_sample IS NULL '
		'	AND feature.name_lower IN ["harvest time", "harvest date"] '
		'	THEN [1] ELSE [] END '
		' 	FOREACH (n IN CASE '
		'		WHEN feature.name_lower = "harvest date"'
		'		AND item.`harvest date` IS NULL '
		'		THEN [1] ELSE [] END '
		'			SET item.`harvest date` = value '
		'			item.time = CASE '
		...
		'	) '
		' 	FOREACH (n IN CASE '
		'		WHEN feature.name_lower = "harvest time" '
		'		AND item.`harvest time` IS NULL '
		'		THEN [1] ELSE [] END '
		'			SET item.`harvest time` = value '
		'			item.time = CASE '
