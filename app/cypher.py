class Cypher:
	...
	upload_FB = (
		...
		' WITH item, trait, person, location, time, value, plot_item_plot '
		...
		# Optional matches so can modify relationships FROM_BRANCH, FROM_LEAF, IS_IN where being overwritten
		' OPTIONAL MATCH '
		'	(item)-[is_in:IS_IN]->(:BlockTrees)<-[:FOR]-(current_block_counter:Counter) WHERE is_in.current = True, '
		# for branch and leaf assignment find current 
		'	(item)-[from_branch_current:FROM_BRANCH]->(:Branch) WHERE from_branch_current.current = True, '
		...
		'	(variety_update:Variety {name_lower: toLower(d.value)}) '
		...
		'			SET '
		'				is_in.current = False '
		'				current_block_counter = current_block_counter -1, '
		'				s1.current = True '
