...
#sample procedures
	...
	tissue_add = (
		' MATCH (:User {username:$username})-[:SUBMITTED]->(:Submissions) '
			' -[:SUBMITTED]->(:SampleDescriptors)-[:SUBMITTED]->(tissues:Tissues) '
		' MERGE (tissue:Tissue {name: $tissue}) '
		' MERGE (tissues)-[s:SUBMITTED]->(tissue)'		
			' ON CREATE SET s.time = timestamp() ' 
		' RETURN tissue '
	)
	storage_add = (
		'MATCH (user:User {username:$username})-[:SUBMITTED]->(:Submissions)  '
			' -[:SUBMITTED]->(:SampleDescriptors)-[:SUBMITTED]->(sm:StorageMethods) '
		' MERGE (storage:Storage {name :$storage}) '
		' MERGE (sm)-[s:SUBMITTED]->(storage) '
			' ON CREATE SET s.time = timestamp() ' 
		' RETURN storage '
	)
	samples_add = (
	...
	upload_check_value = (
		...
		' CASE '
				...
				' THEN CASE '
					' WHEN '
						...
						' size('
							' FILTER ( '
								' n in split(value, ":") '
								' WHERE n in split(trait.categories, "/") '
							' ) '
						' ) '
					' THEN '
						' extract( '
			...
			' WHEN trait.format = "categorical" '
				' THEN CASE '
					' WHEN '
						' value in split(trait.categories, "/") '
						' THEN value '
					' ELSE '
						' False '
					' END '
			' WHEN trait.format = "text" '
				' THEN CASE '
					' WHEN trait.name = "variety" '
						' THEN CASE '
							' WHEN value in varieties '
								' THEN toString(value) '
							' ELSE '
								' False '
							' END '
					' WHEN trait.name = "el frances code" '
						' THEN CASE '
							' WHEN value in variety_codes '
								' THEN toString(value) '
							' ELSE '
								' False '
							' END '
					' WHEN trait.name = "synthetic fertiliser n:p:k ratio" '
						' THEN CASE '
					...
					' WHEN trait.name = "branch id" '
						' THEN CASE '							

					' WHEN trait.name = "leaf id" '
						' THEN CASE '	
					
					' WHEN trait.name = "time harvested (24:00)" '
						' THEN CASE '
			...
			' WHEN trait.format = "percent" '
				' THEN CASE '
					' WHEN '
						' toFloat(replace(value, "%", "")) IS NOT NULL '
			...
			' WHEN trait.format = "location" '
				' THEN CASE '
					' WHEN '
						' size(split(value, ";")) = 2 '
						' AND '
						' toFloat(trim(split(value, ";")[0])) IS NOT NULL ' 
						' AND '
						' toFloat(trim(split(value, ";")[1])) IS NOT NULL ' 
						' THEN '
							' toString(value) '
					' ELSE '
						' FALSE '
			...
			' WHEN trait.format = "date" '
				' THEN CASE '
					' WHEN '
						' size(split(value, "-")) = 3 '
						' AND '
						' size(toFloat(trim(split(value, "-")[0]))) = 4 ' 
						' AND '
						' size(toFloat(trim(split(value, "-")[1])) <3 '
						' AND '
						' size(toFloat(trim(split(value, "-")[1])) >0 '
						' AND '
						' size(toFloat(trim(split(value, "-")[2])) <3 '
						' AND '
						' size(toFloat(trim(split(value, "-")[2])) >0 '
						' THEN '
	...
	upload_FB_plot = (
		...
		' MATCH (plot:Plot {uid:uid}),'
			' (trait:PlotTrait {name:trait_name}), '
	...
	upload_FB_block = (
		...
		' WITH '
			...
			' toLower(trim(csvLine.value)) as value '
		# And identify the plots and traits assessed
		' MATCH (block:Block {uid : uid}) '
			' -[:IS_IN]->( : PlotBlocks) '
			' -[:IS_IN]->(plot : Plot {uid : plot_id}), '
			' (trait : BlockTrait {name : trait_name}), '
			# get the user submission tracking nodes
			' (:User {username : $username}) '
				' -[:SUBMITTED]->( : Submissions) '
				' -[:SUBMITTED]->( : DataSub) '
	...
	upload_FB_tree = (
		# load in the csv
		' LOAD CSV WITH HEADERS FROM $filename as csvLine '
		' MATCH (trait : TreeTrait {name : toLower(trim(csvLine.trait))}), '
			' (varieties : TreeTrait {name:"variety"}), '
			' (codes : TreeTrait {name:"el frances code"}) '
		' WITH '
		 	' toString(toLower(trim(csvLine.value))) as value, '
			' trait as trait, '
			' varieties.category_list as varieties, '
			' codes.category_list as variety_codes, '
			' csvLine as csvLine '
		' WITH '
			 + upload_check_value + ' as value, '
			' trait as trait, '
			' csvLine.location as location, '
			' csvLine.person as person, '
			' csvLine.timestamp as time_fb, '
		...
		' OPTIONAL MATCH (variety_name:Variety) '
			' WHERE '
				' variety_name.name = value '
		' OPTIONAL MATCH (variety_code:Variety) '
			' WHERE '
				' variety_code.el_frances_code = value '
		...
		' FOREACH (n IN CASE WHEN trait.name = "assign to block" THEN [1] ELSE [] END | '
			...
			' SET id._LOCK_ = true '
			#then merge the block by name and increment the counter if the block is created
			' MERGE (block : Block {name : value })-[:IS_IN]->(pb) '
				' ON CREATE SET '
					' id.count = id.count + 1, '
			...
			' MERGE (blocks)-[s3 : SUBMITTED]->(block) '
				' ON CREATE SET s3.time = timestamp(), s3.from_upload = true '
		' ) '
		# if variety trait create link tree to variety
		' FOREACH (n IN CASE '
			' WHEN toLower(trim(trait.name)) = "variety" '
			' AND variety_name.name = value '
			' THEN [1] ELSE [] END | '
		...
		' FOREACH (n IN CASE '
			' WHEN toLower(trait.name) = "el frances code" '
			' AND variety_code.el_frances_code = value '
			' THEN [1] ELSE [] END | '
				# Create per plot per Variety node
	...
	upload_FB_branch = (
		...
		' MATCH (branch : Branch {uid: uid}) '
					' -[ : FROM_TREE]->( : TreeBranches) '
					' -[ : FROM_TREE]->(tree : Tree) '
					' -[ : IS_IN]->( : PlotTrees) '
					' -[ : IS_IN]->(plot : Plot {uid : plot_id}), '
				' (trait : BranchTrait { name: trait_name}), '
			# get the user submission tracking nodes
			' (:User {username : $username}) '
				' -[ : SUBMITTED]->(submissions : Submissions) '
				' -[ : SUBMITTED]->( : DataSub) '
