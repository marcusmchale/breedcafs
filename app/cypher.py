class Cypher:
	...
	upload_fb_check = (
		...
		'		CASE WHEN trait IS NULL THEN False Else True END as trait, '
	...
	upload_fb = (
		...
		'	uid, location, person, time, timestamp, '
		'	value '
		' FOREACH (n in CASE '
		...
		'	item, trait, '
		'	plot_item_plot, tree_item_tree, tree_item_plot, '
		'	person, location, time, timestamp, value '
		...
		'	item, trait, '
		'	plot_item_plot, tree_item_tree, tree_item_plot,  '
		'	current_block_counter, is_in_block, '
		'	from_branch_current, branch_update, from_leaf_current, leaf_update, '
		'	is_variety, variety, variety_update, '
		'	fb, it, '
		'	person, location, '
		'	' + upload_check_value + ' as value, '
		'	timestamp, '
		'	time AS time WHERE time IS NOT NULL '
		' WITH '
		'	item, trait, '
		'	plot_item_plot, tree_item_tree, tree_item_plot,  '
		'	current_block_counter, is_in_block, '
		...
		'	is_variety, variety, variety_update, '
		'	fb, it, '
		'	person, location, time, timestamp, '
		'	value AS value WHERE value IS NOT NULL '
		...
		' FOREACH (n IN CASE '
		'	WHEN trait.level = "tree" '
		'	AND trait.name_lower = "variety name (text)" '
		...
		' FOREACH (n IN CASE '
		'	WHEN trait.level = "tree" '
		'	AND trait.name_lower = "el frances code (text)" '
		'	AND d.value IS NOT NULL '
		'	THEN [1] ELSE [] END | '
		# Create per plot per Variety node
		'		MERGE '
	...
	upload_table = (
		...
		'		item, trait, '
		'		plot_item_plot, tree_item_tree, tree_item_plot, '
		'		person, time, table_date, table_time, value '
		'	MATCH '
		'		(:User { '
		'			username_lower : toLower(trim($username))'
		'		}) '
		...
		'		is_variety, variety, variety_update, '
		'		fb, it, '
		'		person, time, table_date, table_time, '
		'		value AS value WHERE value IS NOT NULL '
		...
		# And give the user feedback on their submission success
		'	WITH '
		'		d, value, item, trait, table_date, table_time '
		'	MATCH '
		'		(user:User)-[:SUBMITTED*..7]->()-[submitted:SUBMITTED]->(d) '
		# And give the user feedback on their submission success
		'	RETURN { '
	...
	get_submissions_range = (
		...
		' MATCH '
			'(:User {username:$username}) '
					...
					' -[:FOR_ITEM]->(sst:SampleSampleTrait) '
					' -[:FOR_ITEM*2]->(:PlotSampleTrait) '
