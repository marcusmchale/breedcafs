class Cypher:
	...
	user_allowed_emails = (
		...
		'	-[: SUBMITTED]->(e: Emails) '
		' RETURN '
		'	[n in e.allowed WHERE NOT n in registered_emails] as user_allowed '
	)
	email_find = (
		' MATCH '
		'	(user: User { '
	...
	remove_allowed_email = (
		...
		'	-[:SUBMITTED]->(: Submissions) '
		'	-[:SUBMITTED]->(e: Emails) '
		' WITH e, [x in $email | toLower(trim(x))] as emails'
		' SET e.allowed = [n in e.allowed WHERE NOT n IN emails] '
		' RETURN emails '
	)
	user_del = (
		' MATCH '
	...
	upload_check_value = (
		...
		'		THEN CASE '
		'			WHEN size([n in split(value, ":") WHERE size(n) > 0]) '
		'				= size([n in split(value, ":") WHERE toLower(trim(n)) in '
		'					[item in input.category_list | toLower(item)]]) '
		'			THEN trim(value) '
		'			ELSE Null '
		'			END '
		...
		'		THEN CASE '
		'			WHEN input.name_lower = "assign tree to block by id" '
		'				THEN CASE '
		'					WHEN toInteger(value) IS NOT NULL '
		'				THEN toInteger(trim(value)) '
		'				ELSE Null '
		'				END '
		'			WHEN input.name_lower = "assign tree to block by name" '
		'				THEN toLower(trim(value)) '
		'			WHEN input.name_lower = "assign sample to block(s) by name" '
		'				THEN CASE '
		'					WHEN split(value, ",") IS NOT NULL THEN '
		'						apoc.coll.sort( '
		'							apoc.coll.toSet( '
		'								[ x in split(value, ",") | toLower(trim(x)) ] '
		'							) '
		'						) '
		'					ELSE Null '
		'					END '						
		...
		'						[x in split(value, ",") '
		'							WHERE toInteger(trim(x)) IS NOT NULL '
		'							OR ( '
		'								size(split(x, "-")) = 2'
		...
		'										CASE '
		'											WHEN size(i) > 1 '
		'											THEN toInteger(trim(i[1])) '
		'											ELSE toInteger(trim(i[0])) '
		'										END '
		'									) '
		'								] | output + r )'
		'							) '
	...
	upload_table_property_check = (
			...
			'	input_name as `Supplied input name`, '
			'	item.uid as UID, '
			'	input.name as `Input variable`, '
			'	input.format as Format, '
			'	input.category_list as `Category list`, '
			'	value as Value, '
	...
	upload_table_curve_check = (
		...
		' 	CASE '
		' 		WHEN a.confirmed '
		' 		THEN CASE WHEN r.y_values THEN r.y_values ELSE [] END '
		' 		ELSE CASE '
		' 			WHEN r IS NOT NULL '
		' 			THEN "ACCESS DENIED" '
		' 			ELSE null '
		' 			END '
		' 		END AS r_y_values, '
		'	CASE WHEN r.x_values THEN r.x_values ELSE [] END as r_x_values, '
		' 	s.time AS `submitted at`, '
		' 	CASE WHEN a.confirmed THEN u.name ELSE p.name END AS user, '
		' 	a.confirmed AS access '
		' WHERE '
		...
		'	input.format as Format, '
		'	input.category_list as `Category list`, '
		'	[i in range(0, size(x_values) - 1) | [x_values[i], y_values[i]]] as Value, '
		'	conflicts as Conflicts '
		' ORDER BY row_index '
		' LIMIT 50 '
	)
	...
	upload_table_property = (
		...
		'				ELSE [n in split(value, ":") WHERE  size(n) > 0 | toLower(trim(n))]'
		'				END '
		...
		'	RETURN '
		'		r.found as Found, '
		'		CASE WHEN access IS NOT NULL THEN user.name ELSE partner.name END as `Submitted by`, '
		...
		'			ELSE "ACCESS DENIED" '
		'			END as Value, '
		'		value as `Uploaded value`, '
		'		CASE WHEN access IS NULL THEN False ELSE True END as Access, '
		'		item.uid as UID, '
		'		input.name as `Input variable`, '
	...
	upload_table_curve = (
		...
		'			ELSE "ACCESS DENIED" '
		'			END as Value, '
		'		[i in range(0, size(x_values) - 1) | [x_values[i], y_values[i]]] as `Uploaded value`, '
		'		CASE WHEN access IS NULL THEN False ELSE True END as Access, '
		'		r.replicate as Replicate, '
		'		r.time as Time, '
		'		item.uid as UID, '
	...
	upload_table_condition = (
		...
		'				start: rr.start, '
		'				end: rr.end, '
		'				existing_value: CASE WHEN rr_access IS NOT NULL THEN rr.value ELSE "ACCESS DENIED" END, '
	...
	get_fields_treecount = (
		...
		'		children: [block IN collect(blocks) WHERE block["name"] IS NOT NULL]'
		'	} as fields '
		' WITH '
		'	country, '
		'	region, '
		...
		'		name: farm.name, '
		'		label: "Farm", '
		'		children: [field IN collect(fields) WHERE field["name"] IS NOT NULL]'
		'	} as farms '
		' WITH '
		'	country, '
		'	{'
		'		name: region.name, '
		'		label:"Region", '
		'		children: [farm IN collect(farms) WHERE farm["name"] IS NOT NULL]'
		'	} as regions '
		' WITH '
		'	{'
		'		name: country.name, '
		'		label:"Country", '
		'		children: [region IN collect (regions) WHERE region["name"] IS NOT NULL]'
		'	} as country '
		' RETURN country '
	)
