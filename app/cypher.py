class Cypher:
	...
	shared_upload_code = (
		...
		'		THEN [1] ELSE [] END | '
		'			MERGE '
		'				(field_item_field)<-[: FROM_FIELD]-(field_trait: FieldItemTrait)-[: FOR_TRAIT]->(trait) '
		'			MERGE '
	...
	match_condition_submission_node = (
		' MATCH (:User {username_lower: toLower($username)}) '
		'	-[: SUBMITTED]->(: Submissions) '
		'	-[: SUBMITTED]->(cs: ConditionSubmissions) '
		' RETURN cs '
	)

	merge_condition_submission_node = (
		' MERGE (:User {username_lower: toLower($username)}) '
		'	-[: SUBMITTED]->(sub: Submissions) '
		' MERGE (sub) '
		'	-[: SUBMITTED]->(: ConditionSubmissions) '
	)
	match_treatment_submission_node = (
		' MATCH (:User {username_lower: toLower($username)}) '
		'	-[: SUBMITTED]->(: Submissions) '
		'	-[: SUBMITTED]->(ts: TreatmentSubmissions) '
		' RETURN ts '
	)
	merge_treatment_submission_node = (
		' MERGE (:User {username_lower: toLower($username)}) '
		'	-[: SUBMITTED]->(sub: Submissions) '
		' MERGE (sub) '
		'	-[: SUBMITTED]->(: TreatmentSubmissions) '
	)
	# check to see if a data exists for that condition in the selected period
	# and if the value is different return the item
	existing_field_condition = (
		' MATCH ' 
		'	(condition: FieldCondition { '
		'		name_lower: toLower($condition) '
		'	}) '
		...
		'	(ic)'
		'	<-[:DATA_FOR]-(d:Data) '
		'	<-[sub: SUBMITTED]-(: ConditionSubmissions)'
		'	<-[: SUBMITTED]-(: Submissions)'
		'	<-[: SUBMITTED]-(u: User)'
		'	WHERE '
		'		d.start < $end '
		...
		'		d.end > $start '
		'	AND d.value <> $value '
		' RETURN d.start as start, d.end as end, d.value as value, sub.time as submitted_at, u.name as user '
	)
	existing_field_tree_treatment = (
		...
		'	<-[: FOR_TREATMENT]-(tc: TreatmentCategory) '
		'	<-[: IN_TREATMENT_CATEGORY]-(tree: Tree) '
		'	-[: IS_IN]->(: FieldTrees) '
		'	-[: IS_IN]->(: Field {uid: $field_uid}) '
		'	WHERE tree.id >= $trees_start AND tree.id <= $trees_end '
		'	AND tc.category <> $treatment_category '
		' MATCH '
		'	(tc)'
		'	<-[sub: SUBMITTED]-(: TreatmentSubmissions)'
		'	<-[: SUBMITTED]-(: Submissions)'
		'	<-[: SUBMITTED]-(u: User)'
		' RETURN '
		'	treatment.name as treatment_name, '
		'	tc.category as treatment_category, '
		'	tree.uid as tree_uid,'
		'	tree.custom_id as tree_custom_id, '
		'	sub.time as submitted_at, '
		'	u.name as user '
	)
	...
	merge_field_condition = (
		...
		'	(:User {username_lower: toLower($username)}) '
		'	-[: SUBMITTED]->(: Submissions) '
		'	-[: SUBMITTED]->(us: ConditionSubmissions) '
		' MATCH '
		'	(field:Field {uid: $field_uid}) '
		' MATCH '
		'	(condition: FieldCondition {name_lower: $condition}) '
		' MERGE '
		'	(condition) '
		'	<-[:FOR_CONDITION]-(ic:ItemCondition) '
		'	-[:FOR_ITEM]->(field) '
		' MERGE '
		'	(us)-[:CONTRIBUTED]->(ic) '
		# Prevent overlapping mismatched records but accept overlapping records with same value 
		# If conflict don't accept (checked in existing_field_condition)
		# if just a resubmission of the same value for the same time period then don't make a new one
		# just link the submission record and add a timestamp to this if not already linked
		' MERGE '
		'	(d:Data { '
		'		start: $start, '
		'		end: $end, '
		...
		' MATCH '
		'	(d) '
		'	<-[s: SUBMITTED]-(: ConditionSubmissions) '
		'	<-[: SUBMITTED]-(: Submissions) '
		'	<-[: SUBMITTED]-(u: User) '
		' RETURN '
		'	d.start as start, '
	...
	merge_field_tree_treatment = (
		' MATCH '
		'	(:User {username_lower: toLower($username)}) '
		'	-[: SUBMITTED]->(: Submissions) '
		'	-[: SUBMITTED]->(us: TreatmentSubmissions) '
		' MATCH '
		'	(field:Field {uid: $field_uid}) '
		'	<-[: IS_IN]-(: FieldTrees) '
		'	<-[: IS_IN]-(tree: Tree) '
		...
		'	-[: FOR_TREATMENT]->(treatment) '
		' MERGE '
		'	(tc: TreatmentCategory {category: $treatment_category})-[: FOR_TREATMENT]->(fit) '
		' MERGE '
		'	(tree)-[: IN_TREATMENT_CATEGORY]->(tc) '
		' MERGE '
		'	(us)-[:CONTRIBUTED]->(fit) '
		' MERGE '
		'	(us)-[s1: SUBMITTED]->(tc) '
		'	ON CREATE SET '
		'		s1.time = timestamp() '
		' WITH tc, treatment, tree '
		' MATCH '
		' (tc) '
		'	<-[s: SUBMITTED]-(: TreatmentSubmissions) '
		'	<-[: SUBMITTED]-(: Submissions) '
		'	<-[: SUBMITTED]-(u: User) '	
		' RETURN '
		'	treatment.name as treatment_name, '
