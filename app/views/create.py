	...
	if 'username' not in session:
		...
		if request.method == 'POST':
			if add_country.submit.data:
				if Fields(add_country.text.data).find_country():
			...
			if add_farm.submit.data:
				if not form.country.data and form.region.data:
					...
					flash('Farm already found: ' + add_farm.text.data + ' in ' + form.region.data + ' of ' + form.country.data )
				else:
					Fields(form.country.data).add_farm(form.region.data, add_farm.text.data)
					flash('Farm submitted: ' + add_farm.text.data + ' in ' + form.region.data + ' of ' + form.country.data )
			if add_plot.submit.data:
				if not form.country.data and form.region.data and form.farm.data:
					...
					flash('Farm already found: ' + add_plot.text.data + ' in ' + form.farm.data + ' of ' + form.region.data + ' of ' + form.country.data )
				else:
					Fields(form.country.data).add_plot(form.region.data, form.farm.data, add_plot.text.data)
					flash('Farm submitted: ' + add_plot.text.data + ' in ' + form.farm.data + ' of ' + form.region.data + ' of ' + form.country.data )
