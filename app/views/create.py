		...
		try:
			...
			if form.validate_on_submit():
				...
				if found:
					...
					added = AddLocations(session['username'], text_country).add_country()
			...
			if form.validate_on_submit():
				if country in ['', 'None']:
					...
					if found:
						...
						added = AddLocations(session['username'], country).add_region(text_region)
						return jsonify({"submitted": added})
			else:
				return jsonify([form.errors])
		except (ServiceUnavailable, SecurityError):
			...
			if form.validate_on_submit():
				if bool({country, region} & {'', 'None'}):
					...
					if found:
						...
						added = AddLocations(session['username'], country).add_farm(region, text_farm)
						return jsonify({"submitted": added})
			else:
				return jsonify([form.errors])
		except (ServiceUnavailable, SecurityError):
			...
			if all([location_form.validate_on_submit(), add_block_form.validate_on_submit()]):
				...
				if field_uid in ['','None']:
					...
					if found:
						...
						added = AddFieldItems(session['username'], field_uid).add_block(text_block)
						return jsonify({"submitted": {'uid': added[0], 'name': added[1]}})
			else:
