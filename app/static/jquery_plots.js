...
//generate blocks.csv
$("#generate_plots_csv").click( function(e) {
	e.preventDefault();
	remove_flash();
	wait_message = "Please wait for file to be generated";
	flash_wait = "<div id='generate_plots_flash' class='flash'>" + wait_message + "</div>";
	$("form").append(flash_wait)
	var generate_plots_csv = $.ajax({
		url: "/generate_plots_csv",
		data: $("form").serialize(),
		type: 'POST',
		success: function(response) {
			if (response.hasOwnProperty('submitted')) {
				flash_submitted = "<div id='generate_plots_flash' class='flash'>" + response.submitted + "</div>";
				$("#generate_plots_flash").replaceWith(flash_submitted);
			} else {
				$("#generate_plots_flash").remove();
